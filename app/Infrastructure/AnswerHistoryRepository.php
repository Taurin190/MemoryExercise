...
class AnswerHistoryRepository implements \App\Domain\AnswerHistoryRepository
{
    ...
    public function save(AnswerHistory $answerHistory)
    {
        ...
        $workbook_history_model = WorkbookHistory::convertOrm($workbook_history);
        $workbook_history_model->user()->associate($workbook_history->getUser());
        $workbook_history_model->workbook()->associate(\App\Workbook::convertOrm($workbook_history->getWorkbook()));

        $workbook_history_model->save();
        foreach ($exercise_history_list as $exercise_history) {
            $exercise_history_model = ExerciseHistory::convertOrm($exercise_history);
